...
class LLNCAProcess:
    ...
    def __init__(self, model_path: str):
        ...
        self.process = multiprocessing.Process(
            ...
            args=(self.model_path, self.process_stop_event, self.frames),
            daemon=True,
        )
    ...
    @staticmethod
    def process_run(model_path: str, stop_event, frames: multiprocessing.Queue):
        ...
        for x, y, xs, frame in llnca.eval():
            if stop_event.is_set():
                break
            x_str = llnca.tokenizer.decode(x) if x != prev_x else x_str
            y_str = llnca.tokenizer.decode(y) if y != prev_y else y_str
            try:
                frames.put(
                    {"x_str": x_str, "y_str": y_str, "frame": frame},
...
if __name__ == "__main__":
    ...
    with dpg.window(tag="main_window", label="llnca eval suite"):  # type: ignore
        dpg.add_button(
